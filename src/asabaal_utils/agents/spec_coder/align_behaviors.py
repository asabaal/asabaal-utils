...
class BehavioralAligner:
    """
    Aligns test behaviors with OpenSpec requirements using sophisticated matching algorithms.
    
    This class provides the core functionality for matching test behaviors extracted from
    test files with specification requirements from OpenSpec documents. It uses multiple
    matching strategies including keyword analysis, function signature matching, and
    validation criteria alignment to determine how well tests cover requirements.
    
    The aligner can identify:
    - Direct matches between tests and requirements
    - Partial matches where tests cover some aspects
    - Additional tests that go beyond specification requirements
    - Missing tests for uncovered requirements
    ...
    Example:
        ...
        >>> spec = aligner.load_spec(Path("spec.yaml"))
        >>> test_behaviors = aligner.load_test_behaviors(Path("test_analysis.json"))
        >>> matches = aligner.align_all_tests(test_behaviors, spec)
        >>> print(f"Found {len(matches)} test-requirement alignments")
    """
    
    def __init__(self):
        """
        Initialize the BehavioralAligner with necessary components.
        
        Example:
            >>> aligner = BehavioralAligner()
            >>> ready = aligner.spec_parser is not None
        """
        self.spec_parser = SpecParser()
    
    def load_test_behaviors(self, analysis_file: Path) -> List[BehaviorInfo]:
        """
        Load test behaviors from a test analysis output file.
        
        This method reads a JSON file containing test analysis results and converts
        them into TestBehavior objects for alignment with requirements. The analysis
        file should be generated by the TestAnalyzer or compatible tools.
        
        Args:
            analysis_file: Path to JSON file containing test behavior analysis.
        ...
        Raises:
            FileNotFoundError: If analysis_file does not exist
            JSONDecodeError: If file contains invalid JSON
            KeyError: If required fields are missing from analysis data
        
        Example:
            >>> aligner = BehavioralAligner()
            >>> behaviors = aligner.load_test_behaviors(Path("test_analysis.json"))
            >>> print(f"Loaded {len(behaviors)} test behaviors")
            >>> for behavior in behaviors:
            ...     print(f"  - {behavior.test_name}: {behavior.implied_behavior}")
        """
    ...
    def _infer_test_type(self, test_name: str) -> str:
        """
        Infer the type of test based on naming conventions and keywords.
        
        This private method analyzes test names to determine the likely test type
        (unit, integration, performance, etc.) based on common naming patterns
        and conventions used in software testing.
        
        Args:
            test_name: Name of the test function or method to analyze.
        
        Returns:
            String indicating the inferred test type:
            - 'integration': For integration, end-to-end, or e2e tests
            - 'performance': For performance, load, or stress tests
            - 'unit': For unit tests or when type cannot be determined
        
        Note:
            This method uses keyword matching on common test naming conventions.
            The inference is heuristic-based and may not always be accurate,
            but provides a reasonable default classification.
        
        Example:
            >>> aligner._infer_test_type("test_user_login")
            'unit'
            >>> aligner._infer_test_type("test_user_registration_integration")
            'integration'
            >>> aligner._infer_test_type("test_api_load_performance")
            'performance'
        """
    ...
    def _calculate_keyword_match(self, test_behavior: BehaviorInfo, requirement: Requirement) -> float:
        """
        Calculate a keyword-based match score between test behavior and requirement.
        
        This private method implements keyword overlap analysis to determine how well
        a test's described behavior matches a requirement's description. It considers
        word overlap, synonyms, and semantic similarity to produce a normalized score.
        
        Args:
            test_behavior: TestBehavior object containing test name, implied behavior,
                          and other test metadata.
            requirement: Requirement object containing description, title, and other
                        requirement details.
        
        Returns:
            Float score between 0.0 and 1.0 indicating keyword match quality:
            - 0.8-1.0: Strong keyword overlap
            - 0.5-0.8: Moderate keyword overlap
            - 0.2-0.5: Weak keyword overlap
            - 0.0-0.2: Little to no keyword overlap
        
        Note:
            The algorithm considers:
            - Direct word matches between test and requirement
            - Word variations and synonyms
            - Contextual relevance of matched keywords
            - Length normalization to avoid bias toward longer texts
        
        Example:
            >>> test_behavior = TestBehavior("test_add", "test.py", "Tests addition functionality", "unit", 0.9)
            >>> requirement = Requirement("REQ-001", "Add Numbers", "Function adds two numbers")
            >>> score = aligner._calculate_keyword_match(test_behavior, requirement)
    ...
    def _calculate_function_match(self, test_behavior: BehaviorInfo, requirement: Requirement) -> float:
        """
        Calculate function name match score between test and requirement.
        
        This private method analyzes how well a test name aligns with a requirement's
        function interface name. It uses string similarity algorithms to determine
        the likelihood that the test is specifically targeting the required function.
        
        Args:
            test_behavior: TestBehavior object containing test name and behavior description.
            requirement: Requirement object that may include function interface specifications.
        
        Returns:
            Float score between 0.0 and 1.0 indicating function name match:
            - 1.0: Exact function name match
            - 0.7-0.9: Strong function name similarity
            - 0.4-0.7: Moderate function name similarity
            - 0.1-0.4: Weak function name similarity
            - 0.0: No function interface specified or no meaningful match
        
        Note:
            This method only applies when the requirement includes function interface
            specifications. If no interface is defined, it returns 0.0.
            The comparison is case-insensitive and considers partial matches.
        
        Example:
            >>> test_behavior = TestBehavior("test_calculate_sum", "test.py", "Tests sum calculation", "unit", 0.9)
            >>> requirement = Requirement("REQ-001", "Calculate Sum", "Adds two numbers", 
            ...                          interface=FunctionInterface("calculate_sum", {"a": "int", "b": "int"}, "int"))
            >>> score = aligner._calculate_function_match(test_behavior, requirement)
            >>> print(f"Function match score: {score:.2f}")
        """
    ...
    def _calculate_validation_match(self, test_behavior: BehaviorInfo, requirement: Requirement) -> float:
    ...
    def align_test_to_requirement(self, test_behavior: BehaviorInfo, requirement: Requirement) -> AlignmentMatch:
        """
        Align a single test behavior with a specific requirement using multiple matching strategies.
        
        This method performs sophisticated analysis to determine how well a test covers
        a particular requirement. It uses keyword matching, function signature analysis,
        and validation criteria comparison to calculate alignment confidence and type.
        
        Args:
            test_behavior: TestBehavior object containing test name, file, implied behavior,
                          test type, and confidence score from previous analysis.
            requirement: Requirement object containing id, title, description, interface,
                        and validation criteria from the OpenSpec specification.
        
        Returns:
            AlignmentMatch object containing:
            - test: The original TestBehavior object
            - requirement: The matched Requirement object (or None for additional tests)
            - confidence: Float confidence score (0.0-1.0) indicating alignment quality
            - match_reason: String explanation of why this match was made
            - alignment_type: One of 'direct', 'partial', 'indirect', 'none', 'additional'
            - is_specified: Boolean indicating if test matches a specification requirement
        
        Example:
            >>> aligner = BehavioralAligner()
            >>> test_behavior = TestBehavior("test_add", "test.py", "Tests addition", "unit", 0.9)
            ...
            >>> print(f"Alignment: {match.alignment_type} (confidence: {match.confidence:.2f})")
    ...
    def align_all_tests(self, test_behaviors: List[BehaviorInfo], spec: OpenSpec) -> List[AlignmentMatch]:
        """
        Align all test behaviors with all requirements in a specification.
        
        This method performs comprehensive alignment analysis by matching each test
        behavior against each requirement in the specification. It identifies the
        best matches and categorizes tests as covering requirements, being additional,
        or having no clear alignment.
        
        Args:
            test_behaviors: List of TestBehavior objects from test analysis.
            spec: OpenSpec object containing all requirements and specification details.
        
        Returns:
            List of AlignmentMatch objects representing all test-requirement alignments.
            The list includes:
            - Direct matches between tests and requirements
            - Partial matches for incomplete coverage
            - Additional tests that go beyond specifications
            - Unmatched tests and requirements
        
        Raises:
            ValueError: If test_behaviors is empty or spec has no requirements
        
        Example:
            >>> aligner = BehavioralAligner()
            >>> spec = aligner.load_spec(Path("api_spec.yaml"))
            >>> test_behaviors = aligner.load_test_behaviors(Path("test_analysis.json"))
            >>> matches = aligner.align_all_tests(test_behaviors, spec)
            >>> 
            >>> # Analyze results
            >>> direct_matches = [m for m in matches if m.alignment_type == 'direct']
            >>> additional_tests = [m for m in matches if m.alignment_type == 'additional']
            >>> print(f"Direct matches: {len(direct_matches)}")
            >>> print(f"Additional tests: {len(additional_tests)}")
        """
        ...
        for test_behavior in test_behaviors:
            ...
            if best_match and best_score >= 0.3:  # Threshold for considering it a real match
                ...
                file_tests_so_far = [t for t in matches if Path(t.test.test_file).name == file_name]
                expected_count = expected_tests_per_file.get(expected_file, 0) if expected_file else 0
                
                if len(file_tests_so_far) < expected_count or expected_count == 0:
                    # This test is within the expected count, or no specific count was specified
    ...
    def generate_alignment_report(self, matches: List[AlignmentMatch]) -> Dict[str, Any]:
        ...
        return {
            'summary': {
                ...
                'alignment_rate': (direct_matches + partial_matches + indirect_matches) / len(specified_tests) if specified_tests else 0
            },
