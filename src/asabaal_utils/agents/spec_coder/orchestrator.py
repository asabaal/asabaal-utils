...
class IntegrationOrchestrator:
    """
    Main integration orchestrator for the complete software development pipeline.
    
    This class manages the end-to-end process from OpenSpec specifications through
    code generation, testing, analysis, and optional healing. It coordinates multiple
    pipeline stages to transform specifications into working, tested code.
    
    The pipeline consists of four main stages:
    1. Spec to Scaffold: Generate basic code structure from specification
    2. Scaffold to Requirements: Extract and analyze requirements
    3. Requirements to Alignment: Align tests with requirements
    4. Alignment to Code: Generate final code with proper test coverage
    
    Attributes:
        base_dir: Base directory for all pipeline operations
        scripts_dir: Directory containing pipeline scripts
        healer_dir: Directory containing healing functionality
        prompts_dir: Directory containing prompt templates
        generated_dir: Directory for generated code output
        spec_file_path: Path to the specification file for current run
    
    Example:
        >>> orchestrator = IntegrationOrchestrator(Path("/my/project"))
        >>> success = orchestrator.run_full_pipeline(Path("spec.yaml"))
        >>> if success:
    ...
    def __init__(self, base_dir: Optional[Path] = None):
        """
        Initialize the IntegrationOrchestrator with the specified base directory.
        
        Args:
            base_dir: Base directory for pipeline operations. If None, uses current
                     working directory. All relative paths will be resolved from here.
        
        Raises:
            OSError: If base directory cannot be accessed or created
    ...
    def run_mode(self, mode: str) -> bool:
        ...
        This method provides different execution modes for the pipeline:
        - 'fresh': Run complete pipeline from specification to code
        - 'heal': Run healing mode on existing code
        - 'test': Run testing mode only
        
        Args:
            mode: Pipeline execution mode. Must be one of 'fresh', 'heal', or 'test'.
        ...
        Example:
            >>> orchestrator = IntegrationOrchestrator()
            >>> success = orchestrator.run_mode('fresh')
            >>> if success:
            ...     print("Fresh pipeline completed")
        """
    ...
    def run_full_pipeline(self, spec_file: Path, output_dir: Optional[Path] = None) -> bool:
        """
        Run the complete multi-stage pipeline from specification to generated code.
        
        This is the main entry point for the full integration pipeline. It executes
        all four stages sequentially:
        1. Stage 1: Convert OpenSpec to code scaffold
        2. Stage 2: Extract requirements from scaffold
        3. Stage 3: Align tests with requirements
        4. Stage 4: Generate final code with proper test coverage
        
        Args:
            spec_file: Path to the OpenSpec YAML specification file containing
                      requirements, interfaces, and validation criteria.
            output_dir: Optional directory for pipeline output. If None, uses
                       'generated_functions' subdirectory of base_dir.
        
        Returns:
            True if all pipeline stages completed successfully, False otherwise.
        
        Raises:
            FileNotFoundError: If spec_file does not exist
            ValueError: If spec_file is not a valid OpenSpec file
    ...
    def _stage1_spec_to_scaffold(self, spec_file: Path, output_dir: Optional[Path] = None) -> bool:
        """
        Stage 1: Convert OpenSpec specification to initial code scaffold.
        
        This stage takes an OpenSpec YAML file and generates basic code structure
        including function signatures, basic implementations, and initial test files.
        The generated scaffold serves as the foundation for subsequent analysis
        and refinement stages.
        
        Args:
            spec_file: Path to the OpenSpec specification file containing requirements,
                      function interfaces, and validation criteria.
            output_dir: Directory where the scaffold will be generated. If None,
                       uses 'generated_functions' subdirectory.
        
        Returns:
            True if scaffold generation completed successfully, False otherwise.
        
        Raises:
            FileNotFoundError: If spec_file does not exist
            ValueError: If spec_file contains invalid OpenSpec format
            OSError: If output directory cannot be created or written to
        
        Example:
            >>> orchestrator = IntegrationOrchestrator()
            >>> success = orchestrator._stage1_spec_to_scaffold(
            ...     Path("api_spec.yaml"), 
            ...     Path("scaffold_output")
            ... )
        """
    ...
    def _stage2_scaffold_to_requirements(self, output_dir: Optional[Path] = None) -> bool:
        """
        Stage 2: Extract and analyze requirements from generated scaffold.
        
        This stage analyzes the code scaffold generated in Stage 1 to extract
        detailed requirements, understand function behaviors, and prepare for
        test alignment. It uses both AST parsing and AI analysis to build a
        comprehensive understanding of the generated code.
        
        Args:
            output_dir: Directory containing the scaffold from Stage 1. If None,
                       uses the default generated_functions directory.
        
        Returns:
            True if requirement extraction completed successfully, False otherwise.
        
        Raises:
            FileNotFoundError: If scaffold directory or expected files don't exist
            ValueError: If scaffold contains invalid or unparseable code
            RuntimeError: If AI analysis fails
        
        Example:
            >>> orchestrator = IntegrationOrchestrator()
            >>> success = orchestrator._stage2_scaffold_to_requirements(
            ...     Path("scaffold_output")
            ... )
        """
        ...
        try:
            ...
            try:
                from .parse_tests import ASTVisitor
...
# Parse test files in the output directory
            ...
            if test_results:
                # Convert ParsedTestInfo to dict format for summarizer
                test_data = []
    ...
    def _stage3_requirements_to_alignment(self, output_dir: Optional[Path] = None) -> bool:
        """
        Stage 3: Align test behaviors with specification requirements.
        
        This stage performs sophisticated analysis to match test behaviors with
        original specification requirements, identifying coverage gaps and strengths.
        It uses both keyword matching and AI-powered semantic analysis to determine
        how well tests cover the specified requirements.
        
        Args:
            output_dir: Directory containing scaffold and analysis from previous stages.
                       If None, uses the default generated_functions directory.
        
        Returns:
            True if alignment analysis completed successfully, False otherwise.
        
        Raises:
            FileNotFoundError: If required analysis files from previous stages don't exist
            ValueError: If alignment data is invalid or incomplete
            RuntimeError: If AI-powered alignment analysis fails
        
        Example:
            >>> orchestrator = IntegrationOrchestrator()
            >>> success = orchestrator._stage3_requirements_to_alignment(
            ...     Path("scaffold_output")
            ... )
        """
 ...
 # Use the spec file that was set during Stage 1, or fall back to searching
            if hasattr(self, 'spec_file_path') and self.spec_file_path and Path(self.spec_file_path).exists():
                spec_file = Path(self.spec_file_path)
                print(f"📋 Using specification from metadata: {spec_file}")
            else:
            ...
            try:
                from .align_behaviors import BehaviorInfo
            except ImportError:
                from align_behaviors import BehaviorInfo
    ...
    def _make_json_serializable(self, obj):
        """
        Convert complex objects to JSON-serializable formats for report generation.
        
        This private method handles the conversion of various object types that may
        be present in alignment reports and other data structures, ensuring they can
        be serialized to JSON format for file output and API responses.
        
        Args:
            obj: Any object that needs to be made JSON-serializable. Can include
                 dataclasses, custom objects, MagicMock objects (from testing),
                 complex nested structures, and other non-serializable types.
        
        Returns:
            JSON-serializable version of the input object. The method handles:
            - Dictionaries: Recursively processes values
            - Lists: Recursively processes elements
            - Dataclasses: Converts to dictionaries
            - Objects with __dict__: Converts to dictionaries
            - MagicMock objects: Converts to string representations
            - Basic types: Returns unchanged
        
        Note:
            This method is essential for saving analysis reports and other pipeline
            outputs to JSON files. It gracefully handles objects that might otherwise
        ...
        Example:
            ...
            ...     name: str
            ...     score: float
            >>> result = TestResult("test_add", 0.95)
            >>> serializable = orchestrator._make_json_serializable(result)
            >>> isinstance(serializable, dict)
            >>> serializable['name'] == "test_add"
        """
    ...
    def _calculate_match_score(self, requirement, test_behavior) -> float:
        """
        Calculate a numerical match score between a requirement and test behavior.
        
        This private method implements a scoring algorithm to quantify how well a test
        covers a specific requirement. It considers multiple factors including keyword
        matching, function name similarity, and behavioral overlap to produce a
        normalized score between 0.0 and 1.0.
        
        Args:
            requirement: Requirement object containing id, title, description, interface,
                        and validation criteria. Used as the reference for what should be tested.
            test_behavior: TestBehavior object containing test name, file, implied behavior,
                          test type, and confidence score. Used as the test implementation to evaluate.
        
        Returns:
            Float score between 0.0 and 1.0 indicating the quality of match:
            - 0.9-1.0: Excellent match (direct alignment)
            - 0.7-0.9: Good match (strong alignment)
            - 0.5-0.7: Moderate match (partial alignment)
            - 0.3-0.5: Weak match (indirect alignment)
            - 0.0-0.3: Poor match (little to no alignment)
        
        Note:
            The scoring algorithm considers:
            - Keyword overlap between requirement description and test behavior
            - Function name similarity (if interface is specified)
            - Test type appropriateness for the requirement
            - Confidence score from test analysis
        
        Example:
            >>> requirement = Requirement("REQ-001", "Add Numbers", "Adds two numbers")
            >>> test_behavior = TestBehavior("test_add_numbers", "test.py", "Tests addition", "unit", 0.9)
            >>> score = orchestrator._calculate_match_score(requirement, test_behavior)
            >>> print(f"Match score: {score:.2f}")  # Should be high for good alignment
        """
    ...
    def _stage4_alignment_to_code(self, output_dir: Optional[Path] = None, dry_run: bool = False) -> bool:
        """
        Stage 4: Generate final code based on alignment analysis.
        
        This final stage uses the alignment analysis from Stage 3 to generate
        refined, production-ready code with proper test coverage. It addresses
        identified gaps, improves implementations, and ensures comprehensive
        test coverage based on the original specification requirements.
        
        Args:
            output_dir: Directory containing outputs from previous stages. If None,
                       uses the default generated_functions directory.
            dry_run: If True, performs analysis without generating actual code.
                    Useful for reviewing what would be generated.
        
        Returns:
            True if final code generation completed successfully, False otherwise.
        
        Raises:
            FileNotFoundError: If alignment analysis files from Stage 3 don't exist
            ValueError: If alignment data is invalid or incomplete
            RuntimeError: If AI-powered code generation fails
        
        Example:
            >>> orchestrator = IntegrationOrchestrator()
            >>> # Dry run to see what would be generated
            >>> success = orchestrator._stage4_alignment_to_code(
            ...     Path("scaffold_output"), 
            ...     dry_run=True
            ... )
            >>> # Actual generation
        ...
        try:
            ...
            if output_dir:
                standard_reports_dir = output_dir / "reports"
            else:
            ...
            # Use the specified output directory or fall back to base directory
            if output_dir:
                work_dir = output_dir
                env['OUTPUT_DIR'] = str(output_dir / "logic_catalog")
            else:
            ...
            env['BASE_DIR'] = str(work_dir)
            
            # Ensure the required directories exist in the correct location
            logic_catalog_dir = work_dir / "logic_catalog"
